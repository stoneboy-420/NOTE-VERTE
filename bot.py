...
async def handle_menu_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action in static and static[action] is not None:
        ...
        # Dédoublonner par nom pour éviter les doublons dans l'affichage
        seen_names = set()
        certified_list = []
        for dep, ps in PROFILS.items():
            for i, p in enumerate(ps):
                if p.get("certified") and p.get("livraison") in (filtre, "les deux") and p["nom"] not in seen_names:
                    seen_names.add(p["nom"])
                    certified_list.append((dep, i, p))
        ...
        keyboard = [
            [InlineKeyboardButton("📸 Abonne-toi sur Instagram", url="https://www.instagram.com/lnv_420?igsh=N3JlZ3hwenJ1b3Q2&utm_source=qr")],
            [InlineKeyboardButton("🏠 Accueil", callback_data="home")],
        ]
        await query.message.delete()
        ...
        keyboard = [
            [InlineKeyboardButton("📸 Instagram", url="https://www.instagram.com/lnv_420?igsh=N3JlZ3hwenJ1b3Q2&utm_source=qr")],
